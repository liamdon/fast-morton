...
import { morton3DEncodeMB } from "../../../dist/index.js";
...
test("valid output x/y/z 1/2/3", (_t) => {
  ...
  for (let i = 0; i < valid.length; i++) {
    for (let j = 0; j < valid[i].length; j++) {
      const coord = [0, 0, 0];
      coord[i] = j + 1;
      const mortonCode = morton3DEncodeMB(...coord);
